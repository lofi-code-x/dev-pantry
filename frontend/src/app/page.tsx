...
function Pill({ children }: { children: React.ReactNode }) {
    return (
        <span className="inline-flex items-center rounded-full border border-border bg-card px-2.5 py-1 text-xs font-medium text-muted-fg">
            {children}
        </span>
    );
}

/** ✅ Hover WITHOUT lift (no translate -> no jump) */
const cardHover =
...
function FeatureCard({
    ...
    return (
        <Link
            href={href}
            className={cn(
                "block rounded-2xl border border-border bg-card p-5 shadow-sm",
                cardHover
            )}
            aria-label={title}
        >
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                    ...
                    <div className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-fg underline underline-offset-4">
                        {cta}
    ...
    return (
        <main className="mx-auto w-full max-w-6xl px-6 py-10">
                ...
                <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    <div className="max-w-2xl">
                        ...
                        <h1 className="mt-4 flex flex-wrap items-center gap-3 text-3xl font-semibold tracking-tight text-fg md:text-4xl">
                            ...
                            <span className="inline-flex items-center rounded-full border border-border bg-[hsl(var(--ring)/0.10)] px-2.5 py-1 text-xs font-semibold text-fg">
                                ALPHA
                            </span>
                        </h1>
                    ...
                    <div className="grid w-full gap-3 md:w-[360px]">
                        <div className={cn("surface p-4", "ring-1 ring-inset ring-border")}>
                            ...
                            <p className="mt-1 text-xs leading-5 text-muted-fg">Категории + поиск + удобная лента постов.</p>
                        </div>
                        ...
                        <div className={cn("surface p-4", "ring-1 ring-inset ring-border")}>
                            ...
                            <p className="mt-1 text-xs leading-5 text-muted-fg">Подборки постов с прогрессом и завершением.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section className={cn("mt-8 surface p-6", "ring-1 ring-inset ring-border")}>
                ...
                <ol className="mt-3 space-y-2 text-sm text-muted-fg">
                    <li className="flex gap-2">
                        <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full border border-border bg-bg text-xs font-semibold text-fg">
                            1
                        </span>
                        <span>
            ...
            <footer className="mt-10 text-center text-xs text-muted-fg">
                StackForge — knowledge & learning hub.
            </footer>
        </main>
    );
}
